...
# max log10 frequency in the corpus
MAX_LOG_FREQ = 4.393374946473126
...
def freq_swift(nu, r, mt=200, eta=-3, NW=10, max_saccades=20, skip_ms=10, beta = 0.5, return_activation_values=False):
    # Processing span normalization constant
    sigma = 1 / (1 + 2 * nu + nu**2)

    # sample word frequencies
    freq = np.random.gamma(0.5, 10_000,size=NW)
    q = np.log10(freq)/ MAX_LOG_FREQ
    a_max = 1 - beta * q
    ...
    while i<max_saccades:
        # 1. Generate fixation duration
        tfix = np.random.gamma(shape * np.mean(a_max), 1/rate)
        traj[i,:] = [time, k + 1, tfix, 1]  # Store trajectory (1-based indexing for Fixation)
        ...
        for _ in range(0, int(tfix), skip_ms):
            time += skip_ms
            a += r * lambda_ * skip_ms / 1000
            a = np.where(a > a_max, a_max, a)
            # Compute word saliencies
            s = np.sin(np.pi * a/a_max)
            s[a >= a_max] = 0
            act.append([time] + s.tolist())
    ...
    if return_activation_values:
        trajectory_df = pd.DataFrame(traj, columns=["Time", "Fixation", "Duration", "Observed"])
        activations_df = pd.DataFrame(act, columns=["Time"] + [i+1 for i in range(NW)])
        return {"trajectory": trajectory_df, "activations": activations_df, "a_max" : a_max}
    else:
        return traj